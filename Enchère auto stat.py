...
def generate_estimations(object_values):
    v = [round(np.random.normal(loc=p, scale=50)) for p in object_values]  # Estimations pour l'ordinateur
    w = [round(np.random.normal(loc=p, scale=50)) for p in object_values]  # Estimations pour Bob
    for i in range(6):
        v[i] = max(v[i], 0)
        w[i] = max(w[i], 0)
    return v, w
...
def select_bob_items(mid, v):
    ...
    for combination in combinations(range(6), 3):
        ...
        if v_sum <= 97 and mid_sum > best_mid_sum:
        ...
        if not best_combination:
            for combination_2 in combinations(range(6), 2):
                ...
                # Vérifie les contraintes : somme des estimations adverses <= 17
                if v_sum <= 98 and mid_sum > best_mid_sum:
                    best_mid_sum = mid_sum
            ...
            if not best_combination:
                for combination_1 in combinations(range(6), 1):
                    ...
                    # Vérifie les contraintes : somme des estimations adverses <= 17
                    if v_sum <= 99 and mid_sum > best_mid_sum:
                        best_mid_sum = mid_sum
...
def auction_game():
    ...
    print(f"Vraies valeurs (cachées)  : {object_values}")
